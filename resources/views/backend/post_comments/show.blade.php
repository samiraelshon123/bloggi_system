      ...
      <div class="table-responsive">
        <table class="table table-hover">
          ...
          <tbody>
                ...
                <tr>
                    <td colspan="4"><a href="{{url('admin/post_show/'.$post->id)}}">{{$post->title}}</a></td>
                </tr>
                <tr>
                    <th>Comments</th>
                ...
                <tr>
                    <td colspan="4">
                        ...
                        @if($post->media->count() > 0)
                            ...
                            <div class="col-2">
                                <img src= "{{ asset('assets/posts/' . $media->file_name) }}" alt="" class="img-fluid">
                            </div>
                            @endforeach
                        @endif
                        </div>
          ...
          <tbody>
            @forelse ($post->comments as $comment)
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="{{url('admin/post_comment_edit/'.$comment->id)}}" class="btn btn-primary"><i class="fa fa-edit"></i></a>
                            ...
                            <form action="{{url('admin/post_comment_destroy/'.$comment->id)}}" method="post" id="comment-delete-{{$comment->id}}">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
