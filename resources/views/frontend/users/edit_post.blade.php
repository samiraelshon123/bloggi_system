...
@section('content')
    <!-- Start Blog Area -->
                ...
                <div class="col-lg-9 col-12">
                   ...
                   {!! Form::model($post, ['url' => 'user/update_post/'.$post->id, 'method' => 'put', 'files' => true]) !!}
       ...
       $(function () {
            ...
            $('#post-images').fileinput({
                ...
                initialPreviewConfig: [
                    ...
                    @if($post->media->count() > 0)
                        @foreach($post->media as $media)
                            {caption: "{{ $media->file_name }}", size: {{ $media->file_size }}, width: "120px", url: "{{ url('user/post_media_destroy', [$media->id, '_token' => csrf_token()]) }}", key: "{{ $media->id }}"},
                        @endforeach
                    @endif
                ],
            })
